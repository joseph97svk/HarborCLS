...
/**
  ...
  *     's' for stream
  *     'd' for datagram
  *  @param	bool _ipv6: if we need a IPv6 socket
  *
 **/
...
void Socket::SSLInitServer(std::string& certificateFileName,
      std::string& keyFileName) {
  bool missingCertFile = !std::filesystem::exists(certificateFileName);
  bool missingKeyFile = !std::filesystem::exists(keyFileName);

  if (missingCertFile || missingKeyFile) {
    std::string message;

    message += missingKeyFile? ("Certificate " + certificateFileName + " does not exist\n") : "";
  ...
  this->SSLLoadCertificates(certificateFileName.c_str(), keyFileName.c_str());
}
